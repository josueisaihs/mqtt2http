...
def validation_pattern(value):
    def validator(value):
        if "#" in value:
            if value.count("#") > 1:
                raise ValidationError(
                    "The topic pattern can only have one wildcard character '#' at the end of the string."
                )
        ...
        if len(value) > 1000:
            raise ValidationError(
                "The topic pattern can only have a maximum of 1000 characters."
            )
        ...
        if "//" in value:
            raise ValidationError(
                "The topic pattern cannot have two slashes '//' together."
            )
